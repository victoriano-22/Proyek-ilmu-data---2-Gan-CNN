...
file_ = h5py.File('/content/input/train_mscale.h5')
# `in_train` has shape (21884, 33, 33, 1) which corresponds to
# 21884 image patches of 33 pixels height & width and 1 color channel
in_train = file_['data'][:] # the training data
out_train = file_['label'][:] # the training labels
file_.close()
...
# In the above code block, at lines 6 and 7, we define our training data loader and validation data 
...
def validate(model, dataloader, epoch):
    ...
    with torch.no_grad():
        for bi, data in tqdm(enumerate(dataloader), total=int(len(val_data)/dataloader.batch_size)):
            ...
            running_psnr += batch_psnr
        outputs = outputs.cpu()
        save_image(outputs, f"/content/outputs/val_sr{epoch}.png")
    final_loss = running_loss/len(dataloader.dataset)
